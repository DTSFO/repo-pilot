# RepoPilot v1.1 Product Specification
...
6. Deterministic and live-provider modes execute the same state machine and persistence contract.
...
- REST API, SSE replay with `Last-Event-ID`, health/readiness endpoints, and Prometheus metrics.
...
- Mandatory dependence on a model vendor, live API, LangGraph, vector database, Redis, or frontend
  framework during tests.

## Release policy

`v1.0.0` remains a frozen historical release and its recorded baseline is not rewritten. `v1.1.0`
is the agentic workflow feature release and freezes only after every v1.1 gate in
`RELEASE_GATES.md` passes against packaged source. Dependency/security maintenance uses patch
versions; behavior or contract changes require a new minor version and a new measured baseline.
