...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/ingest` | 摄取工作区（拒绝路径穿越/符号链接/敏感文件，跳过二进制/超大文件） |
...
RepoPilot v1.3 的“完整”指单用户、自托管、代码仓库只读研究这一产品范围：真实 `StateGraph` 编排、角色内工具循环、证据审核与停滞收敛、全局预算、引用写作、节点级恢复、Provider 可观测性、API、UI、MCP、评测与容器交付形成闭环。它不等于企业级多租户或互联网规模生产系统。
